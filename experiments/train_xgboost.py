...
def train_and_optimize():
    print("=== XGBoost Training & Optuna Optimization ===")
    ...
    try:
        final_model = xgb.XGBClassifier(**final_params, tree_method='hist', device='cuda')
        final_model.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False)
        print("   Final model trained successfully using GPU (cuda).")
    except Exception:
        final_model = xgb.XGBClassifier(**final_params)
        final_model.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False)
        print("   Final model trained successfully using CPU.")
