...
Output: results/main_pipeline/quantitative_summary.md
"""
...
def generate_summary():
    ...
    for model in all_models:
        ...
        for exp in rq1_data['experiments']:
            if exp['size'] == target_size:
                # Support both old format (mae_global) and new N_REPS format (mae_global_mean)
                if 'mae_global_mean' in exp['cte']:
                    cte_g = exp['cte']['mae_global_mean']
                    cte_l = exp['cte']['mae_local_mean']
                else:
    ...
    for model in all_models:
        ...
        for w in windows:
            # Support all format variants: cte_mae_global_mean (N_REPS), cte_mae_global (old)
            if 'cte_mae_global_mean' in w:
                cte_maes.append(w['cte_mae_global_mean'])
                rand_maes.append(w['rand_mae_global_mean'])
            elif 'cte_mae_global' in w:
                cte_maes.append(w['cte_mae_global'])
                rand_maes.append(w.get('rand_mae_global_mean', w.get('rand_mae_global', 0)))
            else:
                t = np.array(w['truth_importance'])
                c = np.array(w.get('cte_importance_mean', w.get('cte_importance', [])))
    ...
    for model in all_models:
        ...
        if 'cte' in rq3_data:
            cte = rq3_data['cte']
            rand = rq3_data['random']
            # Support both old (train_mae) and new N_REPS (train_mae_mean) format
            cte_train = cte.get('train_mae_mean', cte.get('train_mae'))
            cte_val = cte.get('val_mae_mean', cte.get('val_mae'))
            rand_train = rand.get('train_mae_mean', rand.get('train_mae'))
            rand_val = rand.get('val_mae_mean', rand.get('val_mae'))
            if cte_train is not None and rand_train is not None:
                rq3_per_model[model] = {
                    'cte_train': cte_train,
                    'cte_val': cte_val,
