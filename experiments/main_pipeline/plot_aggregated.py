...
def aggregate_rq1(base_dir, model_list):
    ...
    cte_g_std = np.zeros(n_sizes)
    cte_l_std = np.zeros(n_sizes)
    ...
    for exps in all_exps:
        for i, e in enumerate(exps):
            has_cte_std = 'mae_global_mean' in e['cte']
            if has_cte_std:
                cte_g[i] += e['cte']['mae_global_mean']
                cte_l[i] += e['cte']['mae_local_mean']
                cte_g_std[i] += e['cte']['mae_global_std']
                cte_l_std[i] += e['cte']['mae_local_std']
            else:
                cte_g[i] += e['cte']['mae_global']
                cte_l[i] += e['cte']['mae_local']
                
            has_std = 'mae_global_mean' in e['random']
            if has_std:
    ...
    return {
        ...
        'cte_g_std': cte_g_std / n_models, 'cte_l_std': cte_l_std / n_models,
...
def plot_rq1_aggregated(base_dir):
    ...
    def panel(ax, sizes, cte, cte_std, rand, rand_std, title, ylabel, legend=False):
        ax.plot(sizes, cte, marker='o', color=C_CTE, lw=2, label='CTE (mean)' if np.any(cte_std > 0) else 'CTE')
        if np.any(cte_std > 0):
            ax.fill_between(sizes, np.maximum(cte - cte_std, 0), cte + cte_std,
                            alpha=0.2, color=C_CTE, label='CTE ±σ')
        ...
        if np.any(rand_std > 0):
            ax.fill_between(sizes, np.maximum(rand - rand_std, 0), rand + rand_std,
                            alpha=0.2, color=C_RAND, label='Random ±σ')
    ...
    panel(axes[0, 0], tree['sizes'], tree['cte_g'], tree['cte_g_std'], tree['rand_g'], tree['rand_g_std'],
          'A) Tree Models: Global MAE', 'Global MAE', legend=True)
    panel(axes[0, 1], tree['sizes'], tree['cte_l'], tree['cte_l_std'], tree['rand_l'], tree['rand_l_std'],
          'B) Tree Models: Local MAE', 'Local MAE')
    panel(axes[1, 0], linear['sizes'], linear['cte_g'], linear['cte_g_std'], linear['rand_g'], linear['rand_g_std'],
          'C) Linear Models: Global MAE', 'Global MAE')
    panel(axes[1, 1], linear['sizes'], linear['cte_l'], linear['cte_l_std'], linear['rand_l'], linear['rand_l_std'],
          'D) Linear Models: Local MAE', 'Local MAE')

    fig.suptitle('RQ1: CTE vs Random\n'
                 f'(N ≤ {MAX_SIZE}, averaged across model families)',
                 fontweight='bold', y=0.97)
    plt.tight_layout(rect=[0, 0, 1, 0.93])
    out = base_dir / 'rq1_aggregated_alt.png'
...
def plot_rq2_aggregated(base_dir):
    ...
    for name in ALL_MODELS:
        ...
        new_format = 'cte_mae_global_mean' in windows[0]

        maes_cte = []
        maes_rand = []
        for w in windows:
            if new_format:
                maes_cte.append(w['cte_mae_global_mean'])
    ...
    ax1.fill_between(x, np.maximum(avg_cte - std_cte, 0), avg_cte + std_cte,
                     alpha=0.15, color=C_CTE, label='CTE ±σ')
    ax1.plot(x, avg_rand, marker='s', color=C_RAND, lw=2, label='Random (avg)')
    ax1.fill_between(x, np.maximum(avg_rand - std_rand, 0), avg_rand + std_rand,
                     alpha=0.15, color=C_RAND, label='Random ±σ')
    ...
    fig.suptitle('RQ2: Temporal Feature Importance Tracking (N=128)',
                 fontweight='bold', y=1.0)
    plt.tight_layout()
    out = base_dir / 'rq2_aggregated_alt.png'
...
def plot_rq3_aggregated(base_dir):
    ...
    for name in ALL_MODELS:
        ...
        if 'drift_ranking' in data:
            ...
            model_names.append(name)
            has_cte_mean = 'train_mae_mean' in data['cte']
            if has_cte_mean:
                cte_train_maes.append(data['cte']['train_mae_mean'])
                cte_val_maes.append(data['cte']['val_mae_mean'])
            else:
                cte_train_maes.append(data['cte']['train_mae'])
                cte_val_maes.append(data['cte']['val_mae'])
            
            cte_train_stds.append(data['cte'].get('train_mae_std', 0))
            cte_val_stds.append(data['cte'].get('val_mae_std', 0))
    ...
    # Grouped bar chart — CTE vs Random MAE on Train vs Val samples
    pretty = [PRETTY_NAMES.get(n, n) for n in model_names]
    ...
    ct_arr, ct_std = np.array(cte_train_maes), np.array(cte_train_stds)
    cv_arr, cv_std = np.array(cte_val_maes), np.array(cte_val_stds)
    ct_err_lo = np.minimum(ct_std, ct_arr)
    cv_err_lo = np.minimum(cv_std, cv_arr)
    
    ax1.barh(y - 1.5*height, cte_train_maes, height, label='CTE (train samples) ±σ',
             color=C_CTE, alpha=0.7, xerr=[ct_err_lo, ct_std], capsize=2)
    ax1.barh(y - 0.5*height, cte_val_maes, height, label='CTE (val samples) ±σ',
             color=C_CTE, alpha=1.0, xerr=[cv_err_lo, cv_std], capsize=2)
    ...
    ax1.barh(y + 0.5*height, rand_train_maes, height, label='Random (train samples) ±σ',
             color=C_RAND, alpha=0.7, xerr=[rt_err_lo, rt_std], capsize=2)
    ax1.barh(y + 1.5*height, rand_val_maes, height, label='Random (val samples) ±σ',
    ...
    ax1.set_title('RQ3: CTE vs Random (Train vs Val Context)', loc='left', fontweight='bold')
    ax1.grid(True, axis='x', alpha=0.3)
    ax1.legend(loc='best')

    plt.tight_layout()
    out = base_dir / 'rq3_aggregated_alt.png'
    plt.savefig(out, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✓ {out.name}")
...
def _plot_rq3_aggregated_old(base_dir):
    ...
    fig.suptitle('RQ3: Train-Validation Gap', fontsize=16, fontweight='bold', y=1.0)
    plt.tight_layout()
