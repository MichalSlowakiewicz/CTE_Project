...
def run_filar5_rq2():
    print("=== Filar 5 (PermutationSHAP) - RQ2: Concept Drift ===")
    ...
    with multiprocessing.Pool(num_cores) as pool:
        func = partial(explain_samples_permutation, model_predict_proba=predict_fn, bg_data=bg_truth_test)
        shap_truth = np.vstack(list(tqdm(pool.imap(func, batches), total=len(batches), desc="Ground Truth")))
    
    bg_sizes = [50, 100, 200]
    results = []
    
    for size in tqdm(bg_sizes, desc="RQ2 (Filar 5) sizes"):
        bg_cte_past = pd.read_parquet(cache_dir / f'bg_cte_{size}.parquet')
        with multiprocessing.Pool(num_cores) as pool:
            func = partial(explain_samples_permutation, model_predict_proba=predict_fn, bg_data=bg_cte_past)
        ...
        with multiprocessing.Pool(num_cores) as pool:
            func = partial(explain_samples_permutation, model_predict_proba=predict_fn, bg_data=bg_cte_future)
